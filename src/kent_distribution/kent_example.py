...
def test_example_normalization(showplots=False, verbose=False, gridsize=100, print_grid=True):
    ...
    print
    "Calculating the matrix M_ij of values that can be calculated: kappa=%.1f*i+1, beta=%.1f+j*1" % (scale, scale)
    ...
    if print_grid:
        for message, grid in [
            ...
            for i, line in enumerate(grid):
                print
                " ".join(['  x' if n == -1.0 else '%3i' % n for n in line])
...
def test_example_mle(showplots=False):
    for k in [
        ...
        k_me = kent_me(xs)
        print
        "Moment estimation:  k_me =", k_me
        k_mle = kent_mle(xs, warning=sys.stdout)
        print
        "Fitted with MLE:   k_mle =", k_mle
        ...
        if showplots:
            ...
            f.text(0.30, 0.99 - 0.025,
                   values_t % (k.theta * 180 / pi, k.phi * 180 / pi, k.psi * 180 / pi, k.kappa, k.beta))
            f.text(0.30, 0.99 - 0.055,
                   values_t % (k_me.theta * 180 / pi, k_me.phi * 180 / pi, k_me.psi * 180 / pi, k_me.kappa, k_me.beta))
            f.text(0.30, 0.99 - 0.080, values_t % (
            k_mle.theta * 180 / pi, k_mle.phi * 180 / pi, k_mle.psi * 180 / pi, k_mle.kappa, k_mle.beta))
...
def test_example_mle2(num_samples, showplots=False, verbose=False, stepsize=1.0):
    ...
    print
    "Testing various combinations of kappa and beta for", num_samples, "samples."
    bias_var_mse_kappa_me, bias_var_mse_kappa_mle, bias_var_mse_beta_me, bias_var_mse_beta_mle = [list() for i in
                                                                                                  xrange(4)]
    ...
    for beta_ratio in beta_ratios:
        ...
        if verbose:
            print
            "beta (max 2.0) = %s*kappa : kappa (max %.1f) = " % (beta_ratio, max_kappa),
    ...
    for (name, bias_var_mse_mle, bias_var_mse_me, beta_ratio) in [
        ...
        for mse_me, mse_mle, beta_ratio in zip(mses_me, mses_mle, beta_ratios):
            if mse_me < mse_mle * 0.7:
                print
                "MSE of MLE is lower than 0.7 times the moment estimate for %s" % name
                return False
            if beta_ratio >= 0.3:
                if mse_me < mse_mle:
                    print
                    "MSE of MLE is lower than moment estimate for %s with beta/kappa >= 0.3" % name
                    return False
            if beta_ratio > 0.5:
                if mse_me < 5 * mse_mle:
                    print
                    "MSE of MLE is not lower than five times the moment estimate %s with beta/kappa >= 0.5" % name
                    return False
