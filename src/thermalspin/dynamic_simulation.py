...
from thermalspin.simulation import init_simulation_tilted, init_simulation_random, init_simulation_aligned, \
    run_simulation
...
def dynamic_simulation():
    ...
    for opt, arg in opts:
        if opt == '-Hz':
            ...
            m0_x, m0_y, m0_z = arg.split(",")
            S0 = np.array([float(m0_x), float(m0_y), float(m0_z)])
        elif opt in ("--tilted"):
            tilted = True
