...
class SpinSystem:
    ...
    def __init__(self, initial_state, J, D, Hz, T):
        ...
        self.energy = compute_energy(self.state, self.nx, self.ny, self.nz, J, D, Hz)
    ...
    def step(self):
        ...
        """
        s, e, m = compute_step(self.state, self.nx, self.ny, self.nz, self.J, self.D, self.Hz, self.beta, self.energy,
...
@jit(nopython=USE_NUMBA, cache=USE_NUMBA)
def neighbhour_energy(i, j, k, ii, jj, kk, state, J, D):
    ...
    #if D!=0:
        #DMI_term = np.cross(state[i,j,k], state[ii,jj,kk])

    return heisenberg_term + DMI_term


@jit(nopython=USE_NUMBA, cache=USE_NUMBA)
def compute_energy(state, nx, ny, nz, J, D, Hz):
    """
    ...
    for i, j, k in np.ndindex(nx, ny, nz):
        ii = (i + 1) % nx
        energy_counter += neighbhour_energy(i, j, k, ii, j, k, state, J, D)

        jj = (j + 1) % ny
        energy_counter += neighbhour_energy(i, j, k, i, jj, k, state, J, D)

        if nz > 1:
            kk = (k + 1) % nz
            energy_counter += neighbhour_energy(i, j, k, i, j, kk, state, J, D)
...
@jit(nopython=USE_NUMBA, cache=USE_NUMBA)
def compute_step(state, nx, ny, nz, J, D, Hz, beta, energy, total_magnetization):
    """
    Evolve the system computing a step of Metropolis-Hastings Monte Carlo.
    ...
    ii = (i + 1) % nx
    e0 += neighbhour_energy(i, j, k, ii, j, k, state, J, D)

    ii = (i - 1) % nx
    e0 += neighbhour_energy(i, j, k, ii, j, k, state, J, D)

    jj = (j + 1) % ny
    e0 += neighbhour_energy(i, j, k, i, jj, k, state, J, D)
    ...
    if nz > 1:
        kk = (k + 1) % nz
        e0 += neighbhour_energy(i, j, k, i, j, kk, state, J, D)
    ...
    ii = (i + 1) % nx
    e1 += neighbhour_energy(i, j, k, ii, j, k, state,  J, D)

    ii = (i - 1) % nx
    e1 += neighbhour_energy(i, j, k, ii, j, k, state,  J, D)

    jj = (j + 1) % ny
    e1 += neighbhour_energy(i, j, k, i, jj, k, state,  J, D)
    ...
    if nz > 1:
        kk = (k + 1) % nz
        e1 += neighbhour_energy(i, j, k, i, j, kk, state, J, D)
